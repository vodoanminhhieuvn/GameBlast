...
void Object2D::DoCollision(std::shared_ptr<Object2D> object)
{
	...
	if (CheckCollision(std::make_unique<BoxCollider2D>(broadphaseBox), object->boxCollider))
	{
		...
		if ((tag == Tag::Player && object->tag == Tag::Enemy) ||
			...
			(tag == Tag::Player && object->tag == Tag::CheckPoint)|| (tag == Tag::Player && object->tag == Tag::Gate))
		{
...
bool Object2D::IsCollidable(std::shared_ptr<Object2D>& object)
{
	...
	if ((object->tag == Tag::Punji) && ((this->tag == Tag::PlayerBullet) || (this->tag == Tag::EnemyBullet)))
	{
		return false;
	}

	if (this->name == "Eyeball" && (object->tag == Tag::Terrain || object->tag == Tag::Punji))
	{
		return false;
	}
