...
void OverworldJason::OnCollisionEnter(std::shared_ptr<Object2D> object)
{
	...
	{
		/*LOG_INFO("damn bro");
		std::string s = "left :" + std::to_string(leftCollision) + ", right: " + std::to_string(rightCollision) + ",up :" + std::to_string(upCollision) + ", down: " + std::to_string(downCollision);
		LOG_INFO(s);*/
	}
...
void OverworldJason::OnCollisionStay(std::shared_ptr<Object2D> object)
{
	...
	{
		if (Input::GetKey(KeyCode_RETURN))
		{
			//no idea
			SceneManager::ReloadScene("Game Over Screen");
		}
...
void OverworldJason::Shoot()
{
	...
	if (direction == Direction::Up)
	{
		...
		bullet = std::make_shared<IndoorBullet>(0, location.x, location.y);
	}
	if (direction == Direction::Down)
	{
		location = { transform->position.x, transform->position.y, 0.0f };
		bullet = std::make_shared<IndoorBullet>(1, location.x, location.y);
	}
	if (direction == Direction::Right)
	{
		location = { transform->position.x, transform->position.y, 0.0f };
		bullet = std::make_shared<IndoorBullet>(2, location.x, location.y);
	}
	if (direction == Direction::Left)
	{
		location = { transform->position.x, transform->position.y, 0.0f };
		bullet = std::make_shared<IndoorBullet>(3, location.x, location.y);
	}
