...
#include <Assets\Bullets\Enemy\EnemyCannonBullet\CannonBullet.h>
...
void Cannon::Attack()
{
	...
	if (isVerticalAttack == false)
	{
		...
		bullet_1 = std::make_shared<CannonBullet>(0);
		bullet_2 = std::make_shared<CannonBullet>(1);
	}
	else
	{
		...
		bullet_1 = std::make_shared<CannonBullet>(2);
		bullet_2 = std::make_shared<CannonBullet>(3);
	}
...
void Cannon::OnCollisionEnter(std::shared_ptr<Object2D> object)
{

}

void Cannon::TakeDamage(int p_Damage)
{
	LOG_INFO("it's hurt");
	hitPoint -= p_Damage;
