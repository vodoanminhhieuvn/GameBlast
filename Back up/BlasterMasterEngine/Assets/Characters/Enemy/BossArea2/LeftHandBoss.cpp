...
LeftHandBoss::LeftHandBoss(float x, float y)
	: BossPart(x, y)
{
	name = "Left Hand Boss";
}

void LeftHandBoss::CreateResources()
{
	spriteRenderer->sprite = DeviceResources::LoadTexture(ARM_BOSS_TEXTURE_PATH, 0);
	int spriteWidth = 18;
	int spriteHeight = 32;
	KeyFrame keyFrame;
	RECT rect;
	...
	std::shared_ptr<Animation> BossDisappear = std::make_shared<Animation>("Boss Disappear");
	{
		...
		{
			rect.left = 168 + (spriteWidth + 7) * (index % 6);
			rect.top = 32;
	...
	std::shared_ptr<Animation> BossAppear = std::make_shared<Animation>("Boss Appear");
	{
		...
		for (int index = 0; index < 18; index++)
		{
			rect.left = 289 - (spriteWidth + 7) * (index % 6);
			rect.top = 32;
			rect.right = rect.left + spriteWidth;
			rect.bottom = rect.top + spriteHeight;
			keyFrame.rect = rect;
