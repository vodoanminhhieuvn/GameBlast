...
void Teleporter::Update()
{
	if ((leftCollision != 0) || (rightCollision != 0))
	{
		...
		/*std::string s = "1 :" + std::to_string(leftCollision) + ", " + std::to_string(rightCollision);
		LOG_INFO(s);*/
	}
	if ((upCollision != 0) || (downCollision != 0))
	{
		rigidbody->velocity.y = 0;
		/*std::string s = "2 :" + std::to_string(upCollision) + ", " + std::to_string(downCollision);
		LOG_INFO(s);*/
	}
	AttackCheck();
...
void Teleporter::AttackCheck()
{
	...
	{
		if ((abs(transform->position.y - player->transform->position.y) <= detectZoneY) || (abs(transform->position.x - player->transform->position.x) <= detectZoneX))
		{
			isDetect = true;
		}
...
void Teleporter::OnCollisionEnter(std::shared_ptr<Object2D> object)
{
...
void Teleporter::OnCollisionStay(std::shared_ptr<Object2D> object)
{
	if (object->tag == Tag::Player && object->rigidbody->bodyType == Rigidbody::BodyType::Dynamic)
	{
		std::shared_ptr<OverworldJason> overworldJason = std::dynamic_pointer_cast<OverworldJason>(object);
