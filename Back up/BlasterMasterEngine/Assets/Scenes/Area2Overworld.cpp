...
#include <Assets\Terrains\Punji.h>
#include <Assets\Checkpoint\IndoorHorizontalCheckpoint.h>
#include <Assets\Checkpoint\IndoorVerticalCheckpoint.h>
#include <Assets\Checkpoint\Background.h>
...
#include <Assets\Checkpoint\MainWorldGate.h>


#include "Assets/BossRoom/BossRoomArea2.h"
...
void Area2Overworld::CreateScene()
{
	...
	camera = std::make_shared<OrthographicCamera>(121.0f, mapSize.height - 1910.0f, CAMERA_WIDTH, CAMERA_HEIGHT);
	...
	if (xmlMap == NULL)
	{
		LoadXmlMap(OVERWORLDOBJECTS_TMX);
	}
	...
	{
		std::shared_ptr<Object2D> overworldJason = std::make_shared<OverworldJason>(121.0f, mapSize.height - 1886.0f);
...

	{
		...
		std::shared_ptr<Object2D> guardian_1 = std::make_shared<EnemyGuardian>(185.0f, mapSize.height - 1945.0f);
		guardian_1->CreateResources();
		guardian_1->name = "g 1";
		AddObject(guardian_1);
